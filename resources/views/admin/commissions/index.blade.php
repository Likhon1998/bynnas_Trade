...
@section('content')
    <x-page-header title="Commissions" subtitle="Accrued on verified collections · approve for payout">
        <x-slot:description>Default rule: {{ number_format((float) ($rule?->collection_rate_percent ?? 3.5), 2) }}% on verified payments + {{ number_format((float) ($rule?->target_bonus_percent ?? 1), 2) }}% target bonus when monthly target is met.</x-slot:description>
    </x-page-header>
        ...
        <div class="table-wrap">
            <table class="data">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="9" class="muted" style="padding:24px;text-align:center">No commissions yet. Verify a payment on an order with a salesman.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
